...
class Table:
    def __init__(self, path: Path, batch_size: int, table_name: str):
        ...
        self.cur_file = self.table_dir / f"{self.index:d}.tsv"
    ...
    def write(self, json_value: List[Dict[str, Any]]):
        if self.cur_file_writer is None:
            ...
            out_str = "\t".join([k for k,v in sorted(json_value[0].items())])
            self.cur_file_writer.write(out_str + '\n')
        for json_obj in json_value:
            out_str = "\t".join([v for k,v in sorted(json_obj.items())])
            self.cur_file_writer.write(out_str + '\n')
