...
def match_sentence_eav(sentence: str, tokenized_sentence: list, alias: tuple):
    """
    Match a single sentence with a single statement. Condition: (e, v)
    sentence: str
    ...
    for q in q_text:
        for v in v_text:
            if (q != v) and (v not in q) and (q not in stop) and (v not in stop):
                return q, p_alias_list[0], v  # triple text
    return None
...
def get_arg_parser():
    ...
    parser.add_argument('--dir_output', type=str, default="/afs/crc.nd.edu/group/dmsquare/vol2/myu2/ComparisonSentences/data/statement_scoring/news_ev_v1")
...
if __name__ == "__main__":
    ...
    etypes = []
    with open("etype_list_1223_ev.txt") as f:
        for line in f:
