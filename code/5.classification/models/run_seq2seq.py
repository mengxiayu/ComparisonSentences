...
import transformers
from trainer_seq2seq import QuestionAnsweringSeq2SeqTrainer
...
def main():
    ...
    def preprocess_batch(
        ...
        def truncate_text(text):
            ...
            rlt = " ".join(word_list[:data_args.max_seq_length]) if len(word_list) > data_args.max_seq_length else text
            return rlt

        questions = []
        for pair in examples[question_column]:
            questions.append(f"{truncate_text(pair[0])} [SEP] {truncate_text(pair[1])}")
        answers = []
        for ans in examples[answer_column]:
            answers.append(" ".join([f"[CMP]: {x[0]} [SEP] {x[1]}" for x in ans]))
    ...
    def compute_metrics(p: EvalPrediction):
        ...
        for obj in p.label_ids:
            references.append(" ".join([f"[CMP] {x[0]} [SEP] {x[1]}" for x in obj[data_args.answer_column]]))
